...
function SettingsPage({ t, themeColor, setThemeColor }) {
  const colorOptions = [
            ...
            <CardContent>
              <div className="flex gap-4">
                {colorOptions.map(color => (
                  <div key={color.value} onClick={() => setThemeColor(color.value)} className="flex flex-col items-center gap-2 cursor-pointer">
                    <div className={`w-10 h-10 rounded-full ${color.className} flex items-center justify-center`}>
                      {themeColor === color.value && <CheckCircle className="h-5 w-5 text-white" />}
                    </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="p-4 border rounded-lg dark:border-slate-700">
                ...
                <div className="flex items-center justify-between py-1"><label htmlFor="n-email" className="text-sm">{t.emailNotifications}</label><Switch id="n-email" defaultChecked /></div>
                <div className="flex items-center justify-between py-1"><label htmlFor="n-sms" className="text-sm">{t.smsNotifications}</label><Switch id="n-sms" /></div>
              </div>
              <div className="p-4 border rounded-lg dark:border-slate-700">
                <h3 className="font-medium mb-2">{t.newsUpdates}</h3>
                <div className="flex items-center justify-between py-1"><label htmlFor="n-news-email" className="text-sm">{t.emailNotifications}</label><Switch id="n-news-email" defaultChecked /></div>
                <div className="flex items-center justify-between py-1"><label htmlFor="n-news-whatsapp" className="text-sm">{t.whatsappNotifications}</label><Switch id="n-news-whatsapp" /></div>
              </div>
              <div className="p-4 border rounded-lg dark:border-slate-700">
                <h3 className="font-medium mb-2">{t.followedBusiness}</h3>
                <div className="flex items-center justify-between py-1"><label htmlFor="n-follow-email" className="text-sm">{t.emailNotifications}</label><Switch id="n-follow-email" /></div>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <Card className="border-red-500/50 dark:border-red-500/30">
                <CardHeader><CardTitle className="text-red-600 dark:text-red-400">{t.accountActions}</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    <Button variant="outline" className="w-full justify-start">{t.suspendAccount}</Button>
                    <p className="text-xs text-slate-500 mt-1 pl-1">{t.suspendAccountDesc}</p>
                  </div>
                  <div>
                    <Button variant="destructive" className="w-full justify-start">{t.deleteAccount}</Button>
