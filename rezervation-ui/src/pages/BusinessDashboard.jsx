...
function ServicesManagementTab({t, services, onSaveService, business}) {
    ...
    const getStatusBadge = (status) => {
        switch (status) {
            case 'yeni':
                return <Badge variant="secondary">{t.status_new}</Badge>;
            case 'indirim':
                return <Badge
                    className="bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">{t.status_discount}</Badge>;
            case 'zam':
                return <Badge variant="destructive">{t.status_increase}</Badge>;
            default:
                return null;
        }
    };
        ...
        <div>
            <div className="flex justify-between items-center mb-4 gap-2 flex-wrap">
                <div className="flex gap-2 flex-wrap">
                    <Input placeholder={t.searchService} value={filter.q} onChange={e => {
                        setFilter({...filter, q: e.target.value});
                        setCurrentPage(1);
                    }} className="w-auto sm:w-64"/>
                    <Select value={filter.subCategory} onValueChange={v => {
                        setFilter({...filter, subCategory: v});
                        setCurrentPage(1);
                    }}>
                ...
                <Button onClick={() => openModal()} className="bg-primary hover:bg-primary/90"><PlusCircle
                    className="mr-2 h-4 w-4"/> {t.addNewService}</Button>
            </div>
            <Card>
                <CardContent className="p-0">
                    <div className="divide-y dark:divide-slate-800">
                        {paginatedServices.map(service => (
                            <div key={service.id}
                                 className={`flex gap-4 items-center p-3 ${!service.active ? 'opacity-50' : ''}`}>
                                <img src={`https://placehold.co/80x80/e2e8f0/64748b?text=${service.name.slice(0, 1)}`}
                                     alt={service.name} className="w-12 h-12 rounded-md object-cover hidden sm:block"/>
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {service.oldPrice &&
                                                <p className="text-sm line-through text-slate-500">{formatCurrency(service.oldPrice)}</p>}
                                        </div>
                                    </div>
                                </div>
                                <div className="ml-auto flex items-center gap-2 text-sm">
                                    ...
                                    <Badge variant={service.active ? 'default' : 'outline'}
                                           className={service.active ? 'bg-primary/20 text-primary border-primary/30' : ''}>{service.active ? t.active : t.passive}</Badge>
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><Settings
                                            className="h-4 w-4"/></Button></DropdownMenuTrigger>
                                        <DropdownMenuContent>
                                            <DropdownMenuItem onClick={() => openModal(service)}><Edit
                                                className="mr-2 h-4 w-4"/>{t.editService}</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => onSaveService(business.id, {
                                                ...service,
                                            ...
                                            <DropdownMenuItem className="text-red-500"><Trash2
                                                className="mr-2 h-4 w-4"/> Sil</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </div>
                            </div>
            ...
            />
            <AddServiceModal t={t} isOpen={isModalOpen} onClose={() => setModalOpen(false)}
                             onSave={(s) => onSaveService(business.id, s)} service={editingService}
                             business={business}/>
        </div>
    );
}
