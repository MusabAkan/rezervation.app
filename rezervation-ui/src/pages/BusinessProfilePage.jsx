...
import { MapPin, Calendar, MessageSquare, ThumbsUp, MessageCircleMore, ChevronLeft, ChevronRight } from 'lucide-react';
...
function BusinessProfilePage({ t, business, onBook }) {
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
            <Card className="bg-white dark:bg-slate-800 dark:border-slate-700 overflow-hidden">
                <div className="relative w-full h-64 md:h-80 bg-slate-100 dark:bg-slate-800">
                    <img key={currentImageIndex} src={mockImages[currentImageIndex]} alt={`${business.name} gallery image`} className="w-full h-full object-cover animate-in fade-in-25" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent"></div>
                    <div className="absolute inset-0 flex items-center justify-between px-4">
                        <Button variant="secondary" size="icon" className="rounded-full bg-white/80 hover:bg-white dark:bg-slate-900/80 dark:hover:bg-slate-900" onClick={prevImage}><ChevronLeft className="h-5 w-5"/></Button>
                        <Button variant="secondary" size="icon" className="rounded-full bg-white/80 hover:bg-white dark:bg-slate-900/80 dark:hover:bg-slate-900" onClick={nextImage}><ChevronRight className="h-5 w-5"/></Button>
                    </div>
                     <div className="absolute bottom-4 left-4 text-white">
                        <h1 className="text-3xl font-bold drop-shadow-lg">{business.name}</h1>
                        <p className="flex items-center gap-2 drop-shadow-md"><MapPin className="h-4 w-4" />{business.address}</p>
                    </div>
                </div>
                ...
                <CardContent>
                    ...
                    <Tabs defaultValue="about" className="mt-4">
                        ...
                        <TabsContent value="services" className="pt-4">
                            <input placeholder={t.searchService} value={serviceSearchQuery} onChange={e => setServiceSearchQuery(e.target.value)} className="mb-4 w-full p-2 border rounded-md dark:bg-slate-700 dark:border-slate-600" />
                            <div className="space-y-4">
                                {filteredServices.length > 0 ? filteredServices.map(service => (
                                    <div key={service.id || service.name} className="flex gap-4 items-center p-3 rounded-lg border dark:border-slate-700">
                                        <img src={`https://placehold.co/80x80/e2e8f0/64748b?text=${service.name.slice(0,1)}`} alt={service.name} className="w-16 h-16 rounded-md object-cover" />
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                <ServiceIcon name={service.icon} />
                                                <span className="font-semibold">{service.name}</span>
                                            </div>
                                            <p className="text-sm text-slate-500 line-clamp-2">Bu hizmet için kısa açıklama alanı.</p>
                                        </div>
                                        <span className="font-semibold text-lg">{formatCurrency(service.price)}</span>
                                    </div>
                                )) : <p className="text-sm text-center text-slate-500 py-8">Arama kriterlerinize uygun hizmet bulunamadı.</p>}
                            </div>
                        </TabsContent>
                        <TabsContent value="reviews" className="pt-4">
                            <div className="space-y-4">
                                <Review name="Mehmet K." rating={5} text="Harika işçilik, zamanında teslimat. Kesinlikle tavsiye ederim." />
                                <Review name="Selin A." rating={4} text="Genel olarak memnun kaldım, iletişimleri iyiydi." />
                            </div>
                        </TabsContent>
                                ...
                                {relatedPosts.length > 0 ? relatedPosts.map(post => (
                                    <div key={post.id} className="p-3 rounded-lg border dark:border-slate-700">
                                        ...
                                        <div className="flex items-center gap-4 text-sm text-slate-500">
                                            <span className="gap-2 flex items-center"><ThumbsUp className="h-4 w-4" /> {post.likes}</span>
                                            <span className="gap-2 flex items-center"><MessageCircleMore className="h-4 w-4" /> {post.replies} Yanıt</span>
                                        </div>
                                    </div>
                                )) : <p className="text-sm text-slate-500">{business.name} ile ilgili forum aktivitesi bulunmuyor.</p>}
                            </div>
                        </TabsContent>
                        <TabsContent value="location" className="pt-4">
                            <div className="h-80 bg-slate-100 dark:bg-slate-700 rounded-md flex items-center justify-center text-sm text-slate-500">
